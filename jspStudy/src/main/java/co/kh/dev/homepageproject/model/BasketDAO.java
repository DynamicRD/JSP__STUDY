...
public class BasketDAO {
	...
	private final String SELECT_START_END_SQL = " select * from "
		    ...
		    + " from (select * from Basket order by num desc)) "
		    + "where rnum >= ? and rnum <= ?";
	private final String SELECT_START_END_SUBJECT_SQL = " select * from "
			...
			+ "from (select * from Basket order by ref desc, step asc)) where rnum>=? and rnum<=? and subject LIKE ?";
	private final String SELECT_START_END_WRITER_SQL = " select * from "
			+ "(select rownum AS rnum, num, writer, subject, pass, regdate, readcount, ref, step, depth,comments, content, ip "
			+ "from (select * from Basket order by ref desc, step asc)) where rnum>=? and rnum<=? and writer LIKE ?";
	private final String SELECT_START_END_CONTENT_SQL = " select * from "
			+ "(select rownum AS rnum, num, writer, subject, pass, regdate, readcount, ref, step, depth,comments, content, ip "
			+ "from (select * from Basket order by ref desc, step asc)) where rnum>=? and rnum<=? and content LIKE ?";
	private final String SELECT_ADMIN_START_END_SQL = "SELECT * FROM Basket WHERE writer LIKE '%(관리자)'";
	private final String SELECT_COUNT_SQL = "select count(*) as count from Basket";
...
/**************************
	public int selectCountDB(BasketVO bvo) {
		ConnectionPool cp = ConnectionPool.getInstance();
		Connection con = cp.dbCon();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
